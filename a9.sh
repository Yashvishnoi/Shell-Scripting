...
if  [ $pattern_matches - gt 1 ] ; then
    ...
    index=1
    for name in  $(ls -td   *"$1 ") ;  do        			datetime="$(echo $name | cut  -c1 -14| awk    -F. '{ print $5"/"$4" at "$3":"$2":"$1 }')"
        if [ -d  $name ] ; then
    ...
    echo ""
	read-e -p "Which version of $1to restore ('0' to quit)? [1] : "desired
    ...
    if  [  -e  "$destination/$1 " ] ;    then
		echo "ERROR: Already exists in this directory. Cannot overwrite." >&2; exit 1;       		fi
    ...
    if  [  ${answer:= y}  =  "y"   ] ; then
		...
		echo "additional copies retained."       		fi
else  
    if  [ -e "$destination/$1 " ] ;  then
